...
impl Request {
    ...
    pub fn new(base_url: String, timeout: u64, token: Option<String>, auth_method: String) -> Result<Self> {
    ...
    pub async fn execute(
        ...
        let mut request = match method.to_uppercase().as_str() {
            ...
            _ => return Err(HlqueryError::Unknown(format!("Unsupported HTTP method: {}", method))),
        };
        ...
        if status_code == 403 {
            if let Ok(body_json) = serde_json::from_str::<Value>(&body_text) {
                let error_msg = body_json.get("error")
                    .and_then(|e| e.as_str())
                ...
                if error_msg.contains("Demo mode is enabled") || 
                   ...
                   error_msg.contains("Write operations are not allowed") {
                    let message = body_json.get("message")
                        .and_then(|m| m.as_str())
                        .unwrap_or(error_msg);
