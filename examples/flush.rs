...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if collections_before.is_success() {
        let body: serde_json::Value =
            serde_json::from_str(&collections_before.get_body().to_string())?;
        ...
        println!(
            ...
            collections_before.get_status_code()
        );
    }
    ...
    if collections_after.is_success() {
        let body: serde_json::Value =
            serde_json::from_str(&collections_after.get_body().to_string())?;
        if let Some(collections) = body.get("collections").and_then(|c| c.as_array()) {
            ...
            if count_after == 0 {
                ...
                println!(
                    "  ⚠ Warning: Expected 0 collections, but found {}",
                    count_after
                );
            }
        }
        ...
        println!(
            "  ✗ Failed to list collections: {}",
            collections_after.get_status_code()
        );
    }
