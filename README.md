...
Collection fields are part of the collection metadata response. `get_collection_fields()` and `collections().get_fields()` are compatibility aliases for `GET /collections/{name}`; there is no `/fields` route. Language detection is available separately:

```rust
let metadata = client.get_collection("products").await?;
let fields_compat = client.get_collection_fields("products").await?;
let language = client.get_collection_language("products").await?;
```

The document service covers context and query-oriented operations in addition to CRUD and import:

```rust
use serde_json::json;

let documents = client.documents();
let context = documents.context("products", "prod_1", None).await?;
documents.update_by_query("products", json!({
    "filter_by": "active:false",
    "set": { "archived": true }
})).await?;
documents.delete_by_query("products", json!({ "filter_by": "expired:true" })).await?;
let facets = documents.facet_counts("products", "GET", None, None).await?;
let exported = documents.export("products", "POST", Some(json!({"limit": 100})), None).await?;
let suggestions = documents.maybe("products", "GET", None, None).await?;
```

Search supports collection, global, and multi-search routes with explicit methods when needed:

```rust
let searches = vec![json!({"collection":"products", "q":"keyboard", "query_by":"title"})];
client.search_api().multi_search(searches.clone()).await?; // POST default
client.search_api().multi_search_with_method(searches, "GET").await?;
client.search_api().global_search("POST", Some(json!({
    "collection": "products", "q": "keyboard"
})), None).await?;
```

`client.synonyms()`, `stopwords()`, `overrides()`, `aliases()`, `users()`, `keys()`, `modules()`, and `analytics()` expose the corresponding core routes. Upsert methods require an explicit `POST` or `PUT`. System methods cover status/readiness, metrics, RocksDB, storage checks, counters, and repair.
